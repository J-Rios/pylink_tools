...
Script:
    jlink_dump.py
Description:
    This script connect to JLink target MCU and read it memory to dumpt it
...
class TEXT():
    ...
    OPT_OUT_FILE = \
        "Specify output file to save the dump firmware"

    OPT_FLASH_ADDRESS = \
        "Specify MCU memory address to start reading into firmware file"
...
def main(argc, argv):
    ...
    # Dump memory to file
    if not jlink.fw_dump_file(fw_file, address):
        jlink.disconnect()
        return
    # Disconnect JLink
    logger.info("")
